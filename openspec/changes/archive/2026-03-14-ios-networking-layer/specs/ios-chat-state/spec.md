...
### Requirement: Observable chat state manager
`ChatState` SHALL be an `@Observable` class exposing: `messages: [ChatMessage]`, `connectionState: ConnectionState`, `isTyping: Bool`, `gatewayOnline: Bool`.

#### Scenario: SwiftUI view observes messages
- **WHEN** a new message is appended to `ChatState.messages`
- **THEN** any SwiftUI view observing `chatState.messages` SHALL re-render
...
- **THEN** a new `ChatMessage` with `role: .assistant` and `isStreaming: true` SHALL be added to `messages`
...
- **THEN** the tool event in the current message's `toolEvents` array SHALL update from `.start` to `.result`

### Requirement: Send user message
`ChatState` SHALL provide `sendMessage(text:)` that creates a `ChatMessage` with `role: .user`, appends it to `messages`, and sends a `message.inbound` frame to the relay.

#### Scenario: Send text message
- **WHEN** `sendMessage(text: "hello")` is called
