...
The client SHALL connect to a relay URL via `URLSessionWebSocketTask`, appending `/ws/app` to the base URL if not already present.
...
### Requirement: Message dispatch via async stream
The client SHALL expose received messages as an `AsyncStream<ClawChatMessage>`. Messages of type `pong` SHALL be consumed internally and not emitted.

#### Scenario: Message delivery
- **WHEN** the relay sends a `message.stream` frame
- **THEN** the decoded `MessageStream` SHALL appear in the async stream

#### Scenario: Pong suppression
- **WHEN** the relay sends a `{"type":"pong"}` frame
- **THEN** it SHALL NOT appear in the message async stream

### Requirement: Send typed messages
The client SHALL provide `send(_ message: Encodable)` that JSON-encodes and sends via WebSocket.
...
- **THEN** the call SHALL silently drop the message (no throw)

### Requirement: Connection state observation
The client SHALL expose a `connectionState` property of type `ConnectionState` enum (`.disconnected`, `.connecting`, `.connected`) that updates in real time.
